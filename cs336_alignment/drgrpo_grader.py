...
# 2026.1.13 添加一个可以适用于qwen32B模型的格式判断函数
def new_r1_zero_reward_fn(response, ground_truth, fast=True):
    pattern = r"</think>\s*<answer>"
    if re.search(pattern, response) and "</answer>" in response:
